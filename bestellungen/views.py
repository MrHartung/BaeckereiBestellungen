        ...
        if form.is_valid():
            ...
            if user is not None:
                if not user.is_verified_email:
                    messages.error(request, 'Bitte verifizieren Sie zuerst Ihre E-Mail-Adresse.')
                else:
                    login(request, user)
...
def verify_email_view(request, token):
    ...
    try:
        ...
        if user.verify_email(token):
            messages.success(request, 'E-Mail erfolgreich verifiziert! Sie können sich jetzt anmelden.')
            return redirect('login')
        else:
